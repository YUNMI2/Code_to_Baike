...
class Segment:
    ...
    def get_num_same_segment_with_multi_processs(self, num_processs, output_file):
        # assert self.compare_posi_sentence != {}
        # assert len(self.compare_posi_list) == len(self.compare_posi_sentence)
        ...
        for i in range(len(groups)):
            each_process = multiprocessing.Process(target=self.load_sentences_by_posis_and_caculate_similarity_and_write, args=(groups[i], self.compare_file + ".tmp" + str(i),))
            tmp_output_file_list.append(self.compare_file + ".tmp"+ str(i))
    ...
    def load_sentences_by_posis_and_caculate_similarity_and_write(self, list_groups, tmp_file):
        with open(tmp_file, "w", encoding="utf-8") as fw, open(self.compare_file, "r", encoding="utf-8") as fo:
            for each_posi in list_groups:
                ...
                for one_key_word in key_word_list:
                    # print(one_key_word not in self.train_file_words)
                    if one_key_word not in self.train_file_words:
                        key_word_to_write.append(one_key_word)
                        for i in range(len(one_key_word) - 1):
                            # print(one_key_word[i:i+2])
                            num_same_bichar += self.pool_text.count(one_key_word[i:i+2])

                if num_same_bichar > 0:
        ...
        with open(input_file, "r", encoding="utf-8") as fo:
            ...
            while line:
                ...
                if line_strip:
                    ...
                    if info_list != []:
                        assert sentence != "" and key_word_list != [] and num_bichar != 0
                        for one_key_word in key_word_list:
...
if __name__ == "__main__":
    ...
    test.remove_same_bichar_frequency(file_more_zero, file_remove)
